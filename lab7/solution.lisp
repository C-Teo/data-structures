...
(defun ht-delete (key ht)
  (unless (ht-get key ht)(return-from ht-delete nil))
  (let* ((temp (ht-array ht))
        (index (nth-value 1 (ht-get key *ht*)))
        (item (cdr (aref temp index))))
    (setf (aref temp index) (cons key 'DELETED))
    (setf (ht-array ht) temp)
    (decf (ht-count ht)) 
    (return-from ht-delete (values item index))))
